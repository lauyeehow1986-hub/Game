...
- v9.17.3 — Animation retargeting scaffold + PBR material upgrades. New `animationLibrary.ts`: `POSE_CLIP_SLUGS` maps every `BeatPose` to a Mixamo-naming-compatible clip filename; `PoseAnimationDriver` auto-loads + cross-fades clips from `public/3d/anims/` once they exist (no-op until then). `cloneRig` preserves skinned skeletons across instances. PBR upgrades: clearcoat metal on the cath-lab C-arm + MRI gantry, transmission glass on lead-glass control windows + MRI bore + ward daylight pane (these specifically need IBL to look photoreal).
