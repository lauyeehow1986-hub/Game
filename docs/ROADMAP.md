...
| v9.8 | Phaser canvas renderer (swap in for the SVG `<Stage>`); sprites stay the same data shape | ▶ next |
| v9.9 | Embedded MP4 showpiece clips (cath stent deployment, MRI tube) — first hybrid content | planned |
| v9.10 | Locale translation for the walkthrough strings (`zh`, then `ms`, then `ta`) | planned |
| v9.11 | Second walkthrough — stroke pathway (thrombolysis vs thrombectomy) | planned |
